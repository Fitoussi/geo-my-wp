...
class GMW_SD_Class_Query {
    ...
    public function __construct() {
        ...
        $this->formData[ 'orderby' ]  = ( isset( $_GET[ 'orderby' ] ) && !empty( $_GET[ 'orderby' ] ) ) ? $_GET[ 'orderby' ] : 'active';
        $radius                     = str_replace( ' ', '', explode( ',', $this->gmwMD[ 'radius' ] ) );
        $this->formData[ 'radius' ]   = ( count( $radius ) > 1 && isset( $_GET[ 'field_radius' ] ) && !empty( $_GET[ 'field_radius' ] ) ) ? $_GET[ 'field_radius' ] : end( $radius );
    ...
    public function members_directory_form( $search_form_html ) {
        ...
        if ( count( $radius ) > 1 ) :
            ...
            $search_form_html .= '<option value="' . end( $radius ) . '">' . $radiusText . '</option>';
    ...
    function members_query() {
        ...
        } elseif ( $this->formData[ 'orderby' ] == 'newest' || $this->formData[ 'orderby' ] == 'active' ) {
