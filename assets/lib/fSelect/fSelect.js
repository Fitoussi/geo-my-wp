...
(function($) {
        ...
        fSelect.prototype = {
            create: function() {
                ...
                if ('' !== this.settings.noResultsText) {
                    no_results_html = '<div class="fs-no-results fs-hidden">' + this.settings.noResultsText + '</div>';
                }

                var html = '<div class="fs-label-wrap"><div class="fs-label"></div><span class="fs-arrow"></span></div>';
                html += '<div class="fs-dropdown fs-hidden">{search}{no-results}<div class="fs-options">' + choices_html + '</div></div>';
    ...
    $(document).on('click', '.fs-option:not(.fs-hidden, .disabled)', function(e) {
    ...
    $(document).on('keyup', '.fs-search input', function(e) {
        ...
        if ('' != keywords) {
            ...
            $wrap.find('.fs-optgroup-label').each(function() {
                ...
                var num_visible = $(this).closest('.fs-options').find('.fs-option.g' + group + ':not(.fs-hidden)').length;
                if (num_visible < 1) {
                    $(this).addClass('fs-hidden');
                }
            });
    ...
    $(document).on('keydown', function(e) {
        ...
        if (38 == e.which) { // up
            ...
            var $prev = $current.prevAll('.fs-option:not(.fs-hidden, .disabled)');
            ...
            var $current = $wrap.find('.fs-option[data-index=' + window.fSelect.idx + ']');
            if ($current.length < 1) {
                var $next = $wrap.find('.fs-option:not(.fs-hidden, .disabled):first');
            }
            else {
                var $next = $current.nextAll('.fs-option:not(.fs-hidden, .disabled)');
            }
    ...
    function checkNoResults($wrap) {
        var addOrRemove = $wrap.find('.fs-option:not(.fs-hidden)').length > 0;
        $wrap.find('.fs-no-results').toggleClass('fs-hidden', addOrRemove);
    }
