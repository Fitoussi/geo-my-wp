...
?>	
	<ul class="gmw-tabs-wrapper">
		...
		foreach ( $tabs as $key => $title ) {
			echo '<li><a href="#" id="'.sanitize_title( $key ).'" title="' . esc_attr( $title ) . '"  class="gmw-nav-tab" data-name="'.sanitize_title( $key ).'">' . esc_attr( $title ) . '</a></li>';
		}
		?>
	</ul>			
...
function gmw_save_post_custom_fields() {
	...
	wp_safe_redirect( admin_url( 'admin.php?page=gmw-import-export&tab=posts_locator&gmw_notice=&gmw_notice_status=updated' 
	) );
	...
	public function query_locations() {
		...
		foreach ( $results as $post_key => $post ) {
			...
			foreach( $location_fields as $key => $field ) {
				...
				if ( $key == 'latitude' || $key == 'longitude' || empty( $field ) ) {
					continue;
				}
