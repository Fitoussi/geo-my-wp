...
class GMW_Current_Location_Widget extends WP_Widget {
	...
	function form($instance) {
		...
		<p>
	    	<?php echo '<input type="checkbox" value="1" name="'. $this->get_field_name('name_guest').'"'; if ( isset($instance["name_guest"]) ) echo 'checked="checked"'; echo 'class="checkbox" />'; ?>
	...
	function form($instance) {
		...
		<?php 
			foreach ($shortcodes as $shortcode) :
				echo '<option value="' . $shortcode['ID'] . '"'; if ( isset($instance['short_code']) && $instance['short_code'] == $shortcode['ID'] ) echo 'selected="selected"'; echo '>GMW form ID '. $shortcode['ID'] . '</options>';
			endforeach;
		 ?>
		</select>
		</p>
