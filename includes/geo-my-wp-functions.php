...
abstract class GMW {
	...
	/**
	 *  GMW forms
	 *  all existing forms from database
	 */
	protected $forms;
	...
	public function gmw( $results ) {
		...
		if ( $results == false && isset( $this->form['search_form'] ) ) $this->custom_search_form();
...
function gmw_results_map( $gmw ) {
	...
	if ( $gmw['search_results']['display_map'] != "results" ) return;
...
function gmw_get_search_form_address_field( $gmw, $class ) {
	...
	if ( !isset( $gmw['search_form']['address_field']['within'] ) && !empty( $title ) )  $address_field .= '<label for="gmw-address-'.$gmw['ID'].'">' . $gmw['search_form']['address_field']['title'] . '</label>';
	
	$address_field .=  '<input type="text" name="gmw_address[]" id="gmw-address-'.$gmw['ID'].'" class="'.$am.' gmw-address gmw-full-address gmw-address-'.$gmw['ID'].' '.$class.'" value="'; if ( isset( $_GET['gmw_address'] ) ) $address_field .=  str_replace('+', ' ', implode( ' ', $_GET['gmw_address'] ) ); $address_field .=  '" size="35" '; if ( isset( $gmw['search_form']['address_field']['within'] ) ) $address_field .=  'placeholder="'. $title . '"'; $address_field .=  ' />';
...
function gmw_get_search_form_locator_icon( $gmw, $class ) {
	...
	if ( $icon == 'gmw_na' ) return;
		
	$button  = '<div class="gmw-locator-btn-wrapper gmw-locator-btn-wrapper'.$gmw['ID'].'">';

	$button .= 		'<img id="gmw-locate-button'.$gmw['ID'].'" class="gmw-locate-btn '.$class.'" src="'.GMW_IMAGES .'/locator-images/' .$icon. '" />';
...
function gmw_search_form_radius_values( $gmw, $class, $btitle, $stitle ) {
	...
	if ( empty( $stitle ) ) $stitle = ( $gmw['search_form']['units'] == 'imperial' ) ? __('- Miles -','GMW') : __('- Kilometers -','GMW');
	$btitle = ( empty( $btitle ) )  ? __( ' -- Within -- ','GMW' ) : $btitle;

	if ( count( $miles) > 1 ) :
	
		echo '<select id="gmw-distance-select-'.$gmw['ID'].'" class="gmw-distance-select gmw-distance-select-'.$gmw['ID'].' '.$class.'" name="gmw_distance">';
			echo '<option value="'.end( $miles ).'">'; if ( $gmw['search_form']['units']  == 'both' ) echo $btitle; else echo $stitle; echo '</option>';

			foreach ( $miles as $mile ) :
			
				if ( $_GET['gmw_distance'] == $mile ) $mile_s = 'selected="selected"'; else $mile_s = "";
		...
		echo '<input type="hidden" name="gmw_distance" value="'.end( $miles ).'" />';
	endif;
}
...
function gmw_search_form_units( $gmw, $class ) {
	...
	if ( $gmw['search_form']['units'] == 'both' ) :
		...
		$unit_m = $unit_i = false;
		if ( isset( $_GET['gmw_units'] ) && $_GET['gmw_units'] == 'metric' )  $unit_m = 'selected="selected"'; else $unit_i = 'selected="selected"';
