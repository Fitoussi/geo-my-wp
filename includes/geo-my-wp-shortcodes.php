...
class GMW_Current_location {
	...
	public function current_location( $args ) {
		...
		if ( isset( $_COOKIE['gmw_lat'] ) && isset( $_COOKIE['gmw_lng'] ) ) {
			...
			foreach ( explode( ',', $display_by ) as $field )
				if( isset( $_COOKIE['gmw_'.$field] ) ) $userAddress[] = urldecode( $_COOKIE['gmw_'.$field] );
			...
			$location .= '<span class="gmw-cl-title"><a href="#" class="gmw-cl-form-trigger" title="'.__( 'Your Current Location','GMW').'">'; $location .=__('Get your current location','GMW'); $location .= '</a></span>';
		}
				
		if ( !wp_script_is( 'gmw-cl-js', 'enqueue' ) ) wp_enqueue_script( 'gmw-cl-js' );
	...
	public function cl_template() {
		...
		$template .=			'<div> - or - </div>';
		$template .= 			'<div id="gmw-cl-get-location"><a href="#" id="gmw-cl-trigger" >'; $template .= __( 'Get your current location','GMW' ); $template .= '</a></div>';
		$template .=		'</div>';
