...
class GMW_PT_Search_Form_Helper {
	...
	public static function get_taxonomy( $args = array(), $gmw = array() ) {
		...
		if ( 'dropdown' === $args['usage'] ) {
			...
			// select tag.
			$output .= "<select name=\"tax[{$tax_name}][]\" id=\"{$tax_name}-taxonomy-{$id}\" class=\"gmw-form-field gmw-taxonomy {$tax_name}\" data-gmw-dropdown-parent=\"#{$taxonomy->name}-taxonomy-wrapper\">";
