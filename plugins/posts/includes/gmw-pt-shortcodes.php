...
class GMW_Single_Location {
	...
	public function get_single_location( $single ) {
		...
		if ( isset( $additional_info ) || $additional_info != 0 ) :
			...
			if ( in_array( 'address', $additional_info ) ) { $location_info .= '<div class="gmw-address"><span>' . __('Address: ','GMW'); $location_info .= '</span>'; $location_info .= ( !empty( $post_info->formatted_address ) ) ? $post_info->formatted_address : __('N/A','GMW'); $location_info .= '</div>'; }
			if ( in_array( 'phone', $additional_info ) ) { $location_info   .= '<div class="gmw-phone"><span>'. __('Phone: ','GMW'); $location_info .= '</span>'; $location_info .= ( !empty( $post_info->phone ) ) ? $post_info->phone : __('N/A','GMW'); $location_info .= '</div>'; }
			if ( in_array( 'fax', $additional_info ) ) { $location_info     .= '<div class="gmw-fax"><span>'. __('Fax: ','GMW'); $location_info .= '</span>'; $location_info .= ( !empty( $post_info->fax ) ) ? $post_info->fax : __('N/A','GMW'); $location_info .= '</div>'; }
			if ( in_array( 'email', $additional_info ) ) { $location_info   .= '<div class="gmw-email"><span>'. __('Email: ','GMW'); $location_info .= '</span>'; $location_info .= ( !empty( $post_info->email ) ) ? '<a href="mailto:'.$post_info->email.' ">'.$post_info->email.'</a>' : __('N/A','GMW'); $location_info .= '</div>'; }
			if ( in_array( 'website', $additional_info ) ) { $location_info .= '<div class="gmw-website"><span>'. __('Website: ','GMW'); $location_info .= '</span>'; $location_info .= ( !empty( $post_info->website ) ) ? '<a href="http://' . $post_info->website. '" target="_blank">' .$post_info->website. '</a>' : "N/A"; $location_info .= '</div>'; }
			...
			<script>
				...
				jQuery(document).ready(function($) {
				    ...
				    if ( '<?php echo $map; ?>' == 1 ) {
						...
						if ( '<?php echo $post_info->phone; ?>' != '' ) gmwPhone = '<?php echo $post_info->phone; ?>'; else gmwPhone = 'N/A';
						if ( '<?php echo $post_info->fax; ?>' != '' ) gmwFax = '<?php echo $post_info->fax; ?>'; else gmwFax = 'N/A';
						if ( '<?php echo $post_info->email; ?>' != '' ) gmwEmail = '<?php echo $post_info->email; ?>'; else gmwEmail = 'N/A';
						if ( '<?php echo $post_info->website; ?>' != '' ) gmwWebsite = '<a href="http://<?php echo $post_info->website; ?>" target="_blank"><?php echo $post_info->website; ?></a>'; else gmwWebsite = 'N/A';
