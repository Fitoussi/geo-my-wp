...
function gmw_pt_form_get_post_types_dropdown( $gmw, $title, $class, $all ) {
	...
	if ( !isset( $gmw['search_form']['post_types'] ) || ( isset( $gmw['search_form']['post_types'] ) && count( $gmw['search_form']['post_types'] ) == 1 ) ) return;
		
	if ( isset( $title ) && !empty( $title ) ) echo '<label for="gmw-posts-dropdown-'.$gmw['ID'].'">'.$title.'</label>';
	...
	$output = '<select name="gmw_post" id="gmw-posts-dropdown-'.$gmw['ID'].'" class="gmw-posts-dropdown gmw-posts-dropdown-'.$gmw['ID'].' '.$class.'">';
		...
		foreach ( $gmw['search_form']['post_types'] as $post_type ) :
			...
			if ( isset( $_GET['gmw_post'] ) && $_GET['gmw_post'] == $post_type ) $pti_post = 'selected="selected"'; else $pti_post = "";
...
function gmw_pt_query_taxonomies( $tax_args, $gmw ) {
	...
	if ( !isset( $gmw['search_form']['taxonomies'] ) || empty( $gmw['search_form']['taxonomies'] ) ) return $tax_args;
...
function gmw_pt_within( $gmw, $sm, $om, $rm , $wm, $fm, $nm ) {
	...
	if ( isset( $_GET['action'] ) && $_GET['action'] == 'gmw_post' ) $nm = $gmw['org_address'];
...
function gmw_pt_thumbnail( $gmw, $post ) {
	...
	if ( !isset( $gmw['search_results']['featured_image']['use'] ) ) return;
...
function gmw_pt_additional_info( $gmw, $post ) { 
	 ...
	 if( !isset( $gmw['additional_info'] ) ) return; ?>
    ...
    <?php if( isset( $gmw['search_results']['additional_info']['website'] ) ) { ?><div class="gmw-website"><span><?php _e('Website: ','GMW'); ?> </span><?php if ( $post->website ) { ?><a href="http://<?php echo $post->website; ?>" target="_blank"><?php echo $post->website; } else { _e('N/A','GMW'); }?></a></div><?php } ?>
...
function gmw_pt_excerpt($gmw, $post) {
	...
	if ( !isset( $gmw['search_results']['excerpt']['use'] ) ) return;
...
function gmw_pt_driving_distance( $gmw, $post, $class, $title ) {
	...
	if ( !isset( $gmw['search_results']['by_driving'] ) || $gmw['units_array'] == false ) return;
