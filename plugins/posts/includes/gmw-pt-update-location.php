...
function gmw_pt_update_location( $args ) {
	...
	if ( !isset( $post_id ) || $post_id == false || !isset( $address ) && $address == false ) return;
	...
	if ( is_array( $address ) ) :
		...
		if ( !in_array( 'street', $args['address'] ) ) $street = $returned_address['street'];
		if ( !in_array( 'apt', $args['address'] ) ) $apt = $returned_address['apt'];
		if ( !in_array( 'city', $args['address'] ) ) $city = $returned_address['city'];
		$state = $returned_address['state_short'];
		if ( !in_array( 'zipcode', $args['address']) ) $zipcode = $returned_address['zipcode'];
