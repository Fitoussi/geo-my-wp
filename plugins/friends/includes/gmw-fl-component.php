...
class GMW_Location_Component extends BP_Component {
	...
	function __construct() {
		...
		*/
		...
		add_action( 'gmw_friends_shortcode', array( $this, 'search_functions' ), 10, 2 );
		...
		if ( isset( $this->settings['features']['member_location_widget'] ) ) add_action( 'widgets_init', create_function( '', 'return register_widget( "GMW_Member_Location_Widget" );' ) ); 
	...
	protected function create_db() {
		...
		include_once GMW_FL_PATH . 'includes/admin/gmw-fl-db.php';
	...
	protected function update_db() {
		...
		include_once GMW_FL_PATH . 'includes/admin/gmw-fl-update-db.php';
		gmw_fl_db_update();
	...
	public function frontend_register_scripts() {
		...
		wp_register_script( 'gmw-fl-map', GMW_FL_URL . 'assets/js/main-map.js', array( 'jquery' ), GMW_VERSION, true );
		wp_enqueue_style( 'gmw-fl-style', GMW_FL_URL . 'assets/css/style.css' );
	...
	public function search_functions( $form, $results ) {
		...
		include_once GMW_FL_PATH. 'includes/gmw-fl-search-functions.php';
		new GMW_FL_Search_Query( $form, $results );
