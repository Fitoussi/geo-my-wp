...
function gmw_fl_xprofile_fields( $gmw, $class ) {
	...
	if ( ( !isset( $gmw['search_form']['profile_fields'] ) && !isset( $gmw['search_form']['profile_fields_date'] ) ) ) return;
		
	$total_fields = ( isset( $gmw['search_form']['profile_fields'] ) ) ? $gmw['search_form']['profile_fields'] : array();
	if ( isset( $gmw['search_form']['profile_fields_date'] ) ) array_unshift( $total_fields, $gmw['search_form']['profile_fields_date'] );
...
class GMW_FL_Search_Query extends GMW {
	...
	public function query_xprofile_fields( $clauses ) {
		...
		if( isset( $this->form['search_form']['profile_fields_date'] ) && !empty( $this->form['search_form']['profile_fields_date'] ) ) array_unshift( $total_fields, $this->form['search_form']['profile_fields_date'] );
	
		if ( !isset( $total_fields ) || empty( $total_fields ) ) return $clauses;
	...
	public function gmwBpQuery( $gmwBpQuery ) {
		...
		if ( isset( $this->clauses['bp_user_query']['order_by'] ) ) $gmwBpQuery->uid_clauses['orderby'] = $this->clauses['bp_user_query']['order_by'];
                if ( isset( $this->clauses['bp_user_query']['order'] ) ) $gmwBpQuery->uid_clauses['order'] = $this->clauses['bp_user_query']['order'];
